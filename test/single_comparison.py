...
def duibi(daoBanFileName, daoBanDataList, zhengBanDataList, daoBanColEntry, zhengBanColEntry):
    ...
    if len(newDaoBanDataList) > 0 and len(zhengBanDataList) > 0:
        ...
        for a1 in a:
            if a1 == None:
                continue
            daoBanData, zhengbanValue, bookname, author = a1
            if zhengbanValue == '':
                continue
            if bookname != '':
                daoBanData1=[]
                daoBanData1.extend(daoBanData) 
                daoBanData1.append(bookname)
                booknameResultList.append(daoBanData1)
            if author != '':
                daoBanData2=[]
                daoBanData2.extend(daoBanData)
                daoBanData2.append(author)
                authorResultList.append(daoBanData2)
            if zhengbanValue != '':
                booklist.append(zhengbanValue)

        return (booknameResultList, authorResultList, booklist)
    else:
        print('正版书单或盗版书单为空')
        return []


# C:\Users\Administrator\Desktop\11.xlsx
def bookDuibi(daoBanData, daoBanColEntry, zhengBanDataList, zhengBanColEntry):
    bookname_list = ''
    author_list = ''
    zhengbanValue = ''
    value = str(daoBanData[daoBanColEntry - 1])
    # print(value)
    # pattern = '.*' + value + '.*'
    for j in range(0, len(zhengBanDataList)):
        bookname = str(zhengBanDataList[j][zhengBanColEntry - 1])
        author = str(zhengBanDataList[j][zhengBanColEntry])
        obj = bookname in value

        if obj:
            bookname_list = bookname

        obj2 = author in value
        if obj2:
            author_list = author

        if obj or obj2:
            zhengbanValue = zhengBanDataList[j]
            break

    # print(tort_list)
    a = (daoBanData, zhengbanValue, bookname_list, author_list)
    return a
...
def main():
    ...
    saveFileUrl = ''
    while saveFileUrl == '':
        saveFileUrl = input("请输入对比结果保存文件夹路径：")

    booknameResultList, authorResultList, booklist = duibi(filePath,
                                                           daoBanDataList, zhengBanDataList, 1, 1)

    bookNameDuibiFilePath = saveFileUrl+'/与书名对比结果.xlsx'
    authorDuibiFilePath = saveFileUrl+'/与作者对比结果.xlsx'
    zhengbanFilePath = saveFileUrl+'/被侵权的正版作品.xlsx'

    saveResult(booknameResultList, bookNameDuibiFilePath, filePath[0])
    saveResult(authorResultList, authorDuibiFilePath, filePath[0])
    saveResult(booklist, zhengbanFilePath, filePath[0])
